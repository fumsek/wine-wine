...
export const ProductDetail: React.FC<ProductDetailProps> = ({ 
  ...
  const allImages = [...product.images, ...product.images, ...product.images].slice(0, 4);
  ...
  return (
    <div className="fixed inset-0 bg-white z-[60] overflow-y-auto md:relative md:z-auto md:bg-transparent animate-slide-up">
      <div className="max-w-5xl mx-auto px-4 py-6 pb-32 md:pb-8">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            {/* Share Button - Animated icon */}
            <button 
              onClick={handleShare}
              className="p-2"
            >
              <img 
                src="/system-regular-1-share-in-reveal.webp" 
                alt="Partager" 
            ...
            <button 
              ...
              className="flex items-center gap-1.5 text-gray-600 hover:text-wine-900 transition-colors"
            >
              <Icons.Heart 
                size={20} 
                className={isFavorite ? 'fill-wine-900 text-wine-900' : 'fill-none'} 
                strokeWidth={1.5}
              />
              <span className="text-sm text-airbnb-medium">{favoriteCount}</span>
            </button>
          </div>
        </div>

      <div className="space-y-6">
        {/* Images and Desktop Action Buttons */}
               ...
               {allImages.length > 1 && (
                 <>
                   <button
                     ...
                     className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/90 backdrop-blur-md flex items-center justify-center hover:bg-white transition-colors shadow-lg"
                   >
                   ...
                   <button
                     ...
                     className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/90 backdrop-blur-md flex items-center justify-center hover:bg-white transition-colors shadow-lg"
                   >
                     <Icons.ArrowLeft size={20} className="text-gray-700 rotate-180" />
                   </button>
                 </>
               )}
               ...
               {allImages.map((img, idx) => (
                   <div 
                     ...
                     className={`aspect-square rounded-lg overflow-hidden cursor-pointer border-2 transition-colors ${
                       idx === currentImageIndex ? 'border-wine-900' : 'border-transparent hover:border-gray-300'
                     }`}
                   >
          ...
          <div className="hidden md:block md:col-span-5">
            <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm sticky top-24">
              <div className="space-y-3">
                ...
                {product.isTradeable && (
                  <Button fullWidth variant="secondary" size="lg" className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white">
        ...
        {/* Mobile Order: Price first */}
        <div className="md:hidden space-y-4">
          {/* Price - First on mobile */}
          <div className="flex items-baseline gap-2">
            <span className="text-3xl text-airbnb-extra-bold text-wine-900">{product.price} {product.currency}</span>
          </div>

          {/* Title */}
          <h1 className="text-2xl text-airbnb-extra-bold text-gray-900">{product.title}</h1>

          {/* Info (Category, Date, Location) */}
        ...
        <div className="hidden md:block space-y-4">
          ...
          {/* Price */}
          <div className="flex items-baseline gap-2">
            <span className="text-4xl text-airbnb-extra-bold text-wine-900">{product.price} {product.currency}</span>
          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 z-[70] md:hidden shadow-lg">
        <div className="max-w-5xl mx-auto flex gap-3">
          <Button fullWidth size="lg" className="flex-1">Acheter</Button>
          <Button variant="outline" size="lg" className="flex-1">Contacter</Button>
        </div>
      </div>
      </div>
    </div>
